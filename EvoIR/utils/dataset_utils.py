...
class AdaIRTrainDataset(Dataset):
    ...
    def _init_enhance_ids(self):
        temp_ids = []
        image_list = os.listdir(os.path.join(self.args.enhance_dir, 'input/'))
        temp_ids = image_list
    ...
    def _init_rs_ids(self):
        ...
        self.rs_ids = self.rs_ids * 120#120
    ...
    def _get_enhance_name(self, enhance_name):
        gt_name = enhance_name.replace("input", "gt")
        return gt_name
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.enhance_dir, 'input/', sample["clean_id"])).convert('RGB')), base=16)
...
class DerainDehazeDataset(Dataset):
    ...
    def _get_gt_path(self, degraded_name):
        if self.task_idx == 0:
            ...
            gt_name = degraded_name.replace("input", "gt")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"Load error at idx={idx}: {e}")
            # 直接跳过或返回一个 dummy
            return None
...
class RealTestDataset(Dataset):
    ...
    def _init_clean_ids(self):
        ...
        self.clean_ids += [self.args.dehaze_path + id_ for id_ in name_list]
    ...
    def __getitem__(self, clean_id):
        ...
        # noisy_img = crop_img(np.array(Image.open(self.clean_ids[clean_id].replace('target', 'input')).convert('RGB')), base=16)

        # clean_img, noisy_img = self.toTensor(clean_img), self.toTensor(noisy_img)

        return [clean_name], clean_img
